...
public abstract class AbstractBaumanAdapter {
	...
	public void write2(DocumentPart doc, ParseContext ctx) throws FormatAdapterException
	{
			try {
				for(BaumanFormatVersion v:bm_formats){
					if (v.valid_for.equals(DEFAULT_VERSION)){
				...
				ArrayList<DocumentPart> parts = new ArrayList<DocumentPart>();
				parts.add(doc);
				new String[]{JBMDocumentNames.HEADER, JBMDocumentNames.CALCPAGE, JBMDocumentNames.TEXT, JBMDocumentNames.POSITIONS});
				
				for(String partname : new String[]{JBMDocumentNames.HEADER, JBMDocumentNames.CALCPAGE, JBMDocumentNames.TEXT, JBMDocumentNames.POSITIONS}){
					emitStartPart(partname, ctx);
					emitEndPart(partname, ctx);
				}
				doc.getPart(JBMDocumentNames.HEADER);
				emitStart(ctx);
				ctx.lineCounter = 0;
				for (String marker: new String[]{
						k01,k02,k03,k04,k05,k06,k07,k08,k09,k10,k11,k12,
